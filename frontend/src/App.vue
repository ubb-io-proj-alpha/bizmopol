...
const authRequestWithRetry = async (url, body) => {
...
const handleAuth = async () => {
  ...
  if (!email.value || !password.value) {
    authError.value = 'Podaj e-mail i hasło.'
    return
  }
  ...
  try {
    const { response, data } = await authRequestWithRetry(url, body)

    if (!response.ok) {
      authError.value = data?.error || 'Błąd logowania.'
      return
    }
    ...
    authError.value = 'Serwer jeszcze się uruchamia. Spróbuj ponownie za chwilę.'
  } finally {
    ...
    <div class="login-card">
      ...
      <form @submit.prevent="handleAuth" class="login-form">
        ...
        <div class="input-group">
          <label>E-mail</label>
          <input v-model="email" type="email" placeholder="twoj@email.pl" autocomplete="username" required />
        </div>
        ...
        <button type="submit" class="login-btn">
          {{ isAuthLoading ? 'Trwa logowanie...' : (isRegisterMode ? 'Zarejestruj się' : 'Zaloguj się') }}
        </button>
      </form>
